...
if (forEach _list < dayz_maxAnimals) then {
	...
	_PosSelect = 	_PosList select (floor random (forEach _PosList));
	_Pos =  		_PosSelect select 0;
	_list = 		_Pos nearEntities [["CAAnimalBase","Man"],50];
	
	
	if (player distance _Pos < dayz_animalDistance && NOT surfaceIsWater _Pos && (forEach _list <= 1)) then {
