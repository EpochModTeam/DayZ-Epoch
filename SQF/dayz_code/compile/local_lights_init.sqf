...
*/
private ["_lpRange","_hsRange","_nrGen","_genCount","_rndLightsIn","_rndLightsOut","_genClass","_doHouse","_doTower","_doLight","_fnHr","_stHr","_plyr","_ndGen","_trgRng","_rngPlyr","_lightTrig","_lmpCol"];
...
//_doStreet = _this select 3;//Street Lights ? (NOT USED CURRENTLY) - To be used for sparks and sound effects on failed street lights
...
//waitUntil {getPos Player select 0 > 0};//Waits until player out of debug - Required for testing only ! remove all waitUntils !
while {alive player}
do
{
	...
	[_trgRng,player,_rndLightsOut] call axe_returnStreetLights;	
			...
			if(_doLight)then{
				...
				if(_doHouse)then{
				[_hsRange,_lightTrig,_rndLightsOut,_lmpCol,_lpRange,_trgRng] call axeHouseLights;
				};
				/*
				//Not currently used - Will be used for sparks and sound effects
				if(_doStreet)then{
				[_arrStreetLights,_lightTrig,_lmpCol,_rndLightsOut] call axeStreetLights;
				};
				*/
			};
			
			//Decide how to and if to turn lights off
	...
	//}else{
	//Clean up lights after dawn.. Probably more efficient to just leave them on until player leaves server
	//[_rngPlyr,_lightTrig,false,_genCount,_lpRange] call axeDeleteLights;
	};	
sleep 3;
};
